...
const CountPendingSummaryScreen = ({ navigator, route }) => {
  ...
  const handleOnPress = () => {
    console.log("Pressed!")
    // navigator.navigate('Count Pending Bottles')
  }
  ...
  return (
    <Screen style={styles.container}>
      <SectionList
        ...
        renderItem={({ item }) => <ListItem title={`${item.ingrediente}  (${item.cantidad})`} onPress={() => handleOnPress()} />}
